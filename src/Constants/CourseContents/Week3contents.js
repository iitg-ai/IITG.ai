export const Week3contents = [
  {
    text: "Day 1",
    message: "",
  },
  {
    text: `<p>
        SVM
      <br>
      <br>
      SVM is a strong supervised learning technique used for classification and regression applications. It determines the optimum hyperplane for categorizing data points.      </p>`,
    message: "",
  },
  {
    text: `
    <p><u><a target="_blank" href="
    https://youtu.be/efR1C6CvhmE
    ">
    SVM Introduction Video
    </a></u>
    </p>
        `,
    message: `View all three segments of the playlist.
    `,
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://towardsdatascience.com/an-introduction-to-support-vector-machine-3f353241303b
      ">
      Support Vector Machine (blog)
      </a></u>
      </p>
        <br>
        <br>
      <p><u><a target="_blank" href="
      https://www.youtube.com/playlist?list=PLKnIA16_RmvbOIFee-ra7U6jR2oIbCZBL
      ">
      Comprehensive Explanation with Code for SVM
      </a></u>
      </p> 
     `,
    message: `Watch the whole playlist with the code for a complete understanding
    `,
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/fareselmenshawii/svm-from-scratch
      ">
      SVM From Scratch (Notebook) 
      </a></u>
      </p>          `,
    message: "",
  },

  {
    text: "Day 2",
    message: "",
  },
  {
    text: `
      KNN and Introduction to Unsupervised Learning
      <br>
      <br>
      KNN is a simple yet powerful algorithm that can be used for both classification and regression tasks. Now some introduction to Unsupervised Learning, which includes algorithms that learn from unlabelled data.      `,
    message: "",
  },
  {
    text: `
    <p><u><a target="_blank" href="
    https://youtu.be/HVXime0nQeI
    ">
    K-nearest Neighbors Introduction Video
    </a></u>
    </p>
        `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://blog.devgenius.io/implementing-k-nearest-neighbors-from-scratch-in-python-d5eaaf558d49
      ">
      Implementing K Nearest Neighbors (blog)
      </a></u>
      </p> 
            <br>  
            <br>
        <p><u><a target="_blank" href="
        https://www.kaggle.com/code/fareselmenshawii/knn-from-scratch
        ">
        KNN - (Notebook)
        </a></u>
        </p> 
        `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.ibm.com/topics/unsupervised-learning#:~:text=the%20next%20step-,What%20is%20unsupervised%20learning%3F,the%20need%20for%20human%20intervention.
      ">
      What is Unsupervised Learning? (Blog)
      </a></u>
      </p>
      <br>
      <br>
      <p><u><a target="_blank" href="
      https://youtu.be/W01tIRP_Rqs
      ">
      Supervised vs Unsupervised Learning Video
      </a></u>
      </p>
        `,
    message: `Introduction to Unsupervised Learning, with an additional example in the Bonus section.
    `,
  },

  {
    text: "Day 3",
    message: "",
  },
  {
    text: `
        KMeans and Clustering Algorithms
        <br>
          <br>
  
          KMeans clustering is a popular unsupervised learning algorithm for grouping data points into K clusters based on similarity.  
          <br>
          <br>

          Also includes the implementation of several clustering techniques.
                
      `,
    message: "",
  },
  {
    text: `
    <p><u><a target="_blank" href="
    https://www.youtube.com/watch?v=4b5d3muPQmA&list=PLblh5JKOoLUICTaGLRoHQDuF_7q2GfuJF&index=40
    ">
    K-means Introduction Video
    </a></u>
    </p>
    <br>
    <br>
    <p><u><a target="_blank" href="
    https://www.youtube.com/watch?v=AWKCCK5YHsE
    ">
    K-means Clustering Intuition and reasoning Video
    </a></u>
      </p>
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/prashant111/k-means-clustering-with-python
      ">
      K-Means Clustering (Notebook)
      </a></u>
      </p>
      <br>
      <br>
      <p><u><a target="_blank" href="
      https://towardsdatascience.com/k-means-clustering-algorithm-applications-evaluation-methods-and-drawbacks-aa03e644b48a
      ">
      K-means Clustering: Algorithm, Applications (blog) 
      </a></u>
      </p>
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/fareselmenshawii/kmeans-from-scratch
      ">
      KMeans from Scratch
      </a></u>
  
      </p>
      <br>
      <br>
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/azminetoushikwasi/different-clustering-techniques-and-algorithms
      ">
      Different Clustering Algorithms (Notebook) 
      </a></u>
      </p>
      `,
    message: `Also look at the Bonus Section for more clustering algorithms.
    `,
  },

  {
    text: "Day 4",
    message: "",
  },
  {
    text: `
      PCA
      <br>
      <br>
      PCA is a widely used unsupervised learning technique that reduces the dimensionality of high-dimensional data by identifying the most important features or principal components
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.youtube.com/watch?v=FgakZw6K1QQ&list=PLblh5JKOoLUICTaGLRoHQDuF_7q2GfuJF&index=29
      ">
      PCA Introduction Video
      </a></u>
      </p>

      <br>
      <br>
  
      <p><u><a target="_blank" href="
      https://www.analyticsvidhya.com/blog/2020/12/an-end-to-end-comprehensive-guide-for-pca/
      ">
      Principal Component Analysis(PCA)
      </a></u>
      <br>
      Guide Blog
      </p>
      <br>

      `,
    message: ``,
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://towardsdatascience.com/a-one-stop-shop-for-principal-component-analysis-5582fb7e0a9c
      ">
      One-Stop Shop for Principal Component Analysis
      </a></u>
      </p>
      `,
    message: ``,
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/nirajvermafcb/principal-component-analysis-explained
      ">
      Principal Component Analysis explained (Notebook) 
      </a></u>
      </p>
      `,
    message: "",
  },

  {
    text: "Day 5",
    message: "",
  },
  {
    text: `
    Decision Trees
    <br>
      <br>
      Now, back to supervised learning. Decision Trees are a popular supervised learning algorithm for classification and regression tasks. They use a tree-like model to make decisions based on input features.
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://youtu.be/7VeUPuFGJHk?list=PLBq2sVJiEBvA9rPo3IEQsJNI4IJbn81tB
      ">
      Decision Trees Explanation (Video)
      </a></u>
      </p>
      `,
    message: `Watch both parts of Decision Trees.
    `,
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://towardsdatascience.com/building-an-intuition-for-the-decision-tree-algorithm-75e0786e86d
      ">
      Explanation of the Decision Tree Algorithm
      </a></u>
      </p>
      <br>
      <br>
      <p><u><a target="_blank" href="
      https://blog.paperspace.com/decision-trees/
      ">
      Guide to Decision Trees (Blog)
      </a></u>
      </p>
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.kaggle.com/code/prashant111/decision-tree-classifier-tutorial/notebook
      ">
      Using Decision-Tree Classifier (Notebook)
          </a></u>
      </p>
      `,
    message: `Practice Further on Decision Trees with this as well as in Bonus Content.
    `,
  },

  {
    text: "Bonus Section",
    message: "",
  },
  {
    text: `
    Covers SVD, which produces a low-dimensional approximation of a matrix, has uses in Image Compression.
    <br>
    <br>
    Also covers an example of Unsupervised learning.
    `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.accel.ai/anthology/2022/8/17/svd-algorithm-tutorial-in-python
      ">
      SVD Algorithm Tutorial (blog)
      </a></u>
      </p>
      <br>
      <br>
      <p><u><a target="_blank" href="
      https://youtu.be/_So8j8T-E1o
    ">
    SVD Explanatory Video
    </a></u>
      </p>
      `,
    message: `Choose whichever of these you feel most comfortable following.
    `,
  },
  {
    text: `
    <p><u><a target="_blank" href="
    https://www.kaggle.com/code/fareselmenshawii/decision-tree-from-scratch?scriptVersionId=130941860
    ">
    Decision Tree From Scratch
    </a></u>
    (Notebook)
    </p>
      `,
    message: "",
  },
  {
    text: `
      <p><u><a target="_blank" href="
      https://www.youtube.com/watch?v=TP3wdwD8JVY&pp=ygUtYW5vbWFseSBkZXRlY3Rpb24gdXNpbmcgdW5zdXBlcnZpc2VkIGxlYXJuaW5n
          ">
          Unsupervised Learning Example: Anomaly detection using Isolation forest
          </a></u>
      </p>
      
      `,
    message: `A case study for using Unsupervised Learning techniques is offered
    `,
  },

  {
    text: "",
    message: "",
  },
  {
    text: "Assignments",
    message: "",
  },
  {
    text: `
            <p><a target="_blank" href="
            https://forms.gle/GKKhFJLdfZTLNx3EA
            "><u>Quiz : Live Now</u><a></p>
            `,
    message: "",
  },
  {
    text: `
      <p><a target="_blank" href="
      https://drive.google.com/file/d/1TNODmLpXrNhzNYuVkcHwy9CAkVbVSr6x/view?usp=sharing
      "><u>Assignment</u><a></p>
      `,
    message: "",
  },
  {
    text: "",
    message: "",
  },
];
